...
class Net(nn.Module):
    def __init__(self, in_features, dropout=0.25):
        ...
        self.conv2 = nn.Conv1d(self.conv1.out_channels, 1, kernel_size=1, stride=1, padding=0)
        self.conv3 = nn.Conv1d(self.conv2.out_channels, 1, kernel_size=1, stride=1, padding=0)
        self.conv4 = nn.Conv1d(self.conv3.out_channels, 1, kernel_size=1, stride=1, padding=0)
        self.conv5 = nn.Conv1d(self.conv4.out_channels, 1, kernel_size=1, stride=1, padding=0)
        
        self.last = nn.Conv1d(self.conv5.out_channels, 1, kernel_size=1, stride=1, padding=0)
        self.relu = nn.ReLU()
        self.tanh = nn.Tanh()
